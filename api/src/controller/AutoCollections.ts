...
class AutoCollectionsController {
	async createAutoCollection({}, context: IContext) {
		...
		if(phrases.length <= userSettings.settings.autoCollectionSize) {
			const collection = new Collection("Auto", "#dcb499", userSettings.settings.activeProfile, context.auth.userId) as unknown as ICollection;
	...
	async createIntervalCollection({extended = false}, context: IContext): Promise<ICollection> {
		...
		if(
			...
			const collection = new Collection("Interval", "#60a5fa", userSettings.settings.activeProfile, context.auth.userId) as unknown as ICollection;
			collection.isLocked = true;
			collection.isAutoGenerated = true;
			collection.autoGeneratedCollectionMeta = {
				type: "interval"
	...
	async createHardToMemorizeCollection({}, context: IContext) {
		...
		if(phrases.length <= userSettings.settings.autoCollectionSize) {
			const collection = new Collection("Hard to memorize", "#f43f5e", userSettings.settings.activeProfile, context.auth.userId) as unknown as ICollection;
			collection.isLocked = true;
			collection.isAutoGenerated = true;
			collection.phrases = phrases.map((phrase: IPhrase) => phrase.id);
			collection.autoGeneratedCollectionMeta = {
		...
		const collection = new Collection("Hard to memorize", "#fb7185", userSettings.settings.activeProfile, context.auth.userId) as unknown as ICollection;
		collection.isLocked = true;
		collection.isAutoGenerated = true;
		collection.autoGeneratedCollectionMeta = {
			type: "htm"
